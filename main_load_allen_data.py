...
data = EcephysAnalyzer().initialize()
...
if torch.cuda.is_available():
    ...
    args.cuda = False
data = EcephysAnalyzer().initialize(configs=args.n_configs, A=args.A, intensity_mltply=args.intensity_mltply,
...
def train_gradient():
    for Y, access in dataloader:
        optimizer.zero_grad()
        likelihood_term, penalty_term = model.forward(Y, access, args.tau_beta, args.tau_config, args.tau_sigma, stage)
    ...
    for epoch in range(start_epoch, start_epoch + args.num_epochs):
        ...
        if epoch % args.eval_interval == 0 or epoch == start_epoch + args.num_epochs - 1:
            ...
            with torch.no_grad():
                ...
                # print('Truth:', trial_offsets_train.squeeze().permute(1, 0, 2)[0, 0, :])
                # print('Avg:', model_trial_offsets_train[0, 0, :], offsets_train[-1])
                # print('Means:', model.trial_peak_offset_proposal_means.permute(1, 0, 2)[0, 0, :], proposal_means_mses[-1])
