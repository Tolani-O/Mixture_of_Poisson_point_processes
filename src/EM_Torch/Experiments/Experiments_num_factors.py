...
args.notes = f'Learn all with one extra factor. {init} init.'
...
output_str = (
    f"Using CUDA: {args.cuda and torch.cuda.is_available()},\n"
...
# New model for different factors
num_factors = args.L * args.A
model = LikelihoodELBOModel(data.time, num_factors, args.A, args.n_configs, args.n_trial_samples)

start_epoch = 0
args.folder_name = (
    f'Experiment{args.data_seed}_{args.param_seed}_K{args.K}_R{args.n_trials}_A{args.A}_C{args.n_configs}'
...
n_areas = args.A
factors_per_area = num_factors//n_areas
n_neurons = args.K
n_configs = args.n_configs
neurons_assigned = 0
neurons_per_area = n_neurons // n_areas
neuron_factor_access = np.zeros((n_configs, n_neurons, num_factors))
for a in range(n_areas):
    area_start_indx = a * factors_per_area
    if a == n_areas - 1:
        neurons_per_area = n_neurons - neurons_assigned
    neurons_assigned += neurons_per_area
    neuron_factor_access[:, (a * neurons_per_area):((a + 1) * neurons_per_area),
    area_start_indx:(area_start_indx + factors_per_area)] = 1
factor_access_train = torch.tensor(np.transpose(neuron_factor_access, (1, 2, 0)))
factor_access_test = factor_access_train
