...
def get_parser():
    ...
    parser.add_argument('--tau_sigma1', type=int, default=1, help='Value for tau_sigma1')
    parser.add_argument('--tau_sigma2', type=int, default=1, help='Value for tau_sigma2')
    parser.add_argument('--tau_beta', type=int, default=10, help='Value for tau_beta')
    parser.add_argument('--tau_budget', type=int, default=1, help='Value for tau_tau_budget')
...
def plot_outputs(latent_factors, warped_factors, stim_time, output_dir, folder, epoch, batch=10):
    ...
    plt.figure()
    for l in range(L):
        plt.plot(stim_time, latent_factors[l, :] + l, label=f'Factor [{l}, :]')
        plt.title(f'Factors')
    plt.ylim(bottom=0)
