...
class ModelData:
    ...
    def randomly_initialize_parameters(self, n_factors=6, n_trials=15, n_configs=40, n_trial_samples=11, n_config_samples=9):
        # fixed values
        self.config_peak_offset_presamples = torch.normal(0, 1, (n_config_samples, n_configs, 2 * n_factors))
        self.trial_peak_offset_presamples = torch.normal(0, 1, (n_trial_samples, n_trials * n_configs, 2 * n_factors)).view(n_trial_samples, n_trials, n_configs, 2 * n_factors)

        #paremeters
        self.beta = torch.nn.Parameter(torch.rand(n_factors, self.B.shape[0]), requires_grad=False).double()
        self.alpha = torch.nn.Parameter(torch.rand(n_factors), requires_grad=False)
        self.theta = torch.nn.Parameter(torch.rand(n_factors), requires_grad=False)
        self.pi = torch.nn.Parameter(torch.rand(n_factors), requires_grad=False)
        self.config_peak_offset_stdevs = torch.nn.Parameter(torch.rand(2 * n_factors), requires_grad=False)
        bounds = 0.05
        matrix = torch.tril(torch.empty((2 * n_factors, 2 * n_factors)).uniform_(-bounds, bounds))
        matrix.diagonal().add_(0.1)  # Ensure diagonal elements are positive
        # Make it a learnable parameter
        self.trial_peak_offset_covar_ltri = torch.nn.Parameter(matrix, requires_grad=False)
        # solely to check if the covariance matrix is positive semi-definite
        # trial_peak_offset_covar_matrix = torch.mm(self.trial_peak_offset_covar_ltri, self.trial_peak_offset_covar_ltri.t())
        ...
        # std_dev = torch.sqrt(torch.diag(trial_peak_offset_covar_matrix))
        # corr = torch.diag(1/std_dev) @ trial_peak_offset_covar_matrix @ torch.diag(1/std_dev)

    def warp_all_latent_factors_for_all_trials(self, derivs=True):
        ...
        offsets = torch.add(transformed_trial_peak_offsets.unsqueeze(1), transformed_config_peak_offsets.unsqueeze(1).unsqueeze(0))
        avg_peak_times = avg_peak_times.unsqueeze(0).unsqueeze(0).unsqueeze(0).unsqueeze(0)
        s_new = torch.add(avg_peak_times, offsets) # offset peak time
        left_landmarks = self.time[torch.cat([torch.tensor([self.left_landmark1]), torch.tensor([self.left_landmark2])]).repeat_interleave(s_new.shape[-1] // 2).expand_as(s_new)]
        right_landmarks = self.time[torch.cat([torch.tensor([self.right_landmark1]), torch.tensor([self.right_landmark2])]).repeat_interleave(s_new.shape[-1] // 2).expand_as(s_new)]
        ...
        warped_factors, sum_RT_Y_times_warped_bases, sum_RT_warped_factors_times_warped_bases = (
            self.warp_latent_factors(torch.exp(self.beta.detach()), avg_peak_times, left_landmarks, right_landmarks, s_new, derivs))
        return warped_factors, sum_RT_Y_times_warped_bases, sum_RT_warped_factors_times_warped_bases

    def warp_latent_factors(self, gamma, avg_peak_times, left_landmarks, right_landmarks, trial_peak_times, derivs=True):
        ...
        early = early.unsqueeze(-1).unsqueeze(-1).unsqueeze(-1).unsqueeze(-1).unsqueeze(-1).expand((-1,) + tuple(warped_time.shape[1:-1]) + (6,))
        mid = self.time[self.right_landmark1:self.left_landmark2]
        mid = mid.unsqueeze(-1).unsqueeze(-1).unsqueeze(-1).unsqueeze(-1).unsqueeze(-1).expand((-1,) + tuple(warped_time.shape[1:-1]) + (6,))
        late = self.time[self.right_landmark2:]
        late = late.unsqueeze(-1).unsqueeze(-1).unsqueeze(-1).unsqueeze(-1).unsqueeze(-1).expand((-1,) + tuple(warped_time.shape[1:-1]) + (6,))
        ...
        for l in range(gamma.shape[0]):
            ...
            warped_factor_l = torch.from_numpy(spl(l_warped_time))
            ...
            if derivs:
                ...
                for p in range(len(self.Basis_elements)):
                    ...
                    if len(evaluate_at) <= self.degree:
                        ...
                        basis_p = np.broadcast_to(spline(points)[:,None,None,None,None], np.array([evaluate_at.shape[0]]+list(l_warped_time.shape[1:])))
                    else:
                        points = l_warped_time[evaluate_at]
                        basis_p = np.nan_to_num(spline(points.flatten())).reshape(points.shape)
                    basis_p = torch.from_numpy(np.copy(basis_p))
                    sum_RT_Y_times_warped_bases_item = torch.einsum('ktrc,tmnrc->kmnc', self.Y[:,evaluate_at,:,:], basis_p)
                    Y_bases.append(sum_RT_Y_times_warped_bases_item)
                    sum_RT_warped_factors_times_warped_bases_item = torch.einsum('tmnrc,tmnrc->mnc', warped_factor_l[evaluate_at,:,:,:,:], basis_p)
                    factor_bases.append(sum_RT_warped_factors_times_warped_bases_item)
                sum_RT_Y_times_warped_bases.append(torch.stack(Y_bases))
                sum_RT_warped_factors_times_warped_bases.append(torch.stack(factor_bases))
        warped_factors = torch.stack(warped_factors)
        if derivs:
            sum_RT_Y_times_warped_bases = torch.stack(sum_RT_Y_times_warped_bases)
            sum_RT_warped_factors_times_warped_bases = torch.stack(sum_RT_warped_factors_times_warped_bases)
        return warped_factors, sum_RT_Y_times_warped_bases, sum_RT_warped_factors_times_warped_bases
